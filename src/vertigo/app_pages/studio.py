...
def _library_options(*, exclude: set[str] | None = None) -> dict[str, str]:
    exclude = exclude or set()
    assets = library.newest_first(library.filter(kinds=[config.KIND_IMAGE]))
    return {
        f"{asset.name} · {ui.relative_time(asset.created_at)}": asset.id
        for asset in assets
        if asset.id not in exclude
    }
...
def _reference_tray() -> list[str]:
    ...
    with st.popover(i18n.t("studio.refs.from_library"), icon=":material/photo_library:"):
        options = _library_options(exclude=set(refs))
        if options:
            chosen = st.multiselect(
                i18n.t("studio.refs.assets"), list(options), key="studio_ref_picker"
            )
            if st.button(i18n.t("studio.refs.add_selected"), key="studio_ref_add"):
                for label in chosen:
                    refs.append(options[label])
...
def _render_generate() -> None:
    ...
    with left:
        ...
        with st.expander(i18n.t("studio.parameters"), expanded=True):
